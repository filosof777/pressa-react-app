...
function Dropdowns({ result }) {
  ...
  function filterHandler() {
    let test = result.find((element) => 
    element.category_name === filterOptions.category
    ).category_id
    if (filterOptions.category !== 'Kategoriya') {
      if (filterOptions.spiker !== 'Spiker') {
        (async function () {
          const {data} = await axios.get(`https://dream-team-n1.herokuapp.com/api/cards?categories=${test}&author=${filterOptions.spiker}&status=${filterOptions.status === 'Online' ? true : false}`)
  ...
  return (
    <div style={style}>
      <Dropdown>
        ...
        <Button className='btn' onClick={() => {filterHandler()}}>Qidiruv</Button>
    </div>
  );
}
